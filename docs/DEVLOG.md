...
## v1.10.2 - 临时邮箱平台化与外部池统计收敛

发布日期：2026-04-03

### 新增功能

- 新增临时邮箱服务层与 provider 平台化支撑，控制器、路由与外部 API 的边界更清晰，便于后续继续扩展不同临时邮箱接入方。
- 新增 CF Worker 多域名同步能力与设置页同步按钮，域名配置可从前端设置面板直接刷新，减少手工切换与配置漂移。
- 新增邮箱池项目维度复用控制与状态统计口径，`used` 状态开始纳入池内统计，降低对接方对成功完成状态的歧义处理成本。
- 新增前端轮询与列表交互收敛，标准模式、简洁模式和分页体验继续统一，批量轮询不再在错误时机误触发。

### 修复

- 修复 Outlook OAuth、CSRF 恢复、回跳与 token 绑定链路中的回归问题，避免异常回调或旧参数导致导入流程中断。
- 修复外部池 `success -> used` 状态映射、`get_stats` 统计结果以及相关数据库断言，保证完成后账号状态与统计面板一致。
- 修复批量轮询、账号选择状态、语言切换重渲染以及简洁模式相关回归，降低页面切换时的状态丢失概率。
- 修复通知分发、Telegram 推送、临时邮箱图片渲染与刷新失败提示文案不一致的问题，恢复前端反馈的一致性。

### 重要变更

- 版本号已同步为 `1.10.2`，应用 UI、系统接口和对外 API 返回的版本信息继续由 `outlook_web.__version__` 统一驱动。
- 外部邮箱池与临时邮箱的对外接口继续收口到受控路径，内部实现进一步整理为更清晰的 service / repository 分层。
- 当前仓库不是 Tauri 工程，不包含 `Cargo.toml`、`package.json`、MSI 或 NSIS 构建链路；本次发布仍沿用 Docker 镜像 tar 与源码 zip 作为正式产物。

### 测试/验证

- 自动化测试：`python -m unittest discover -s tests -v`
  - 结果：`Ran 803 tests in 189.102s`
  - 状态：全部通过
- 构建验证：`docker build -t outlook-email-plus:v1.10.2 .`
  - 状态：成功
  - 镜像摘要：`sha256:4933cfaca70c854f5cf31fbdf868d0e775345895abf8c6dfd21882ae8d8c0da4`
- 发布产物：
  - `dist/outlook-email-plus-v1.10.2-docker.tar`（202,635,776 bytes）
  - `dist/outlookEmailPlus-v1.10.2-src.zip`（2,878,029 bytes）
